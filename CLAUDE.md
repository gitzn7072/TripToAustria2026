...
- **Live weather, sun & how-to-dress** — on load the script fetches [Open-Meteo](https://open-meteo.com) (free, no key, CORS) per day from `data-lat/lon/date`. On success it (1) swaps the weather section's static 🌦️ summary icon for the **live condition icon** and appends the Hebrew condition + temp range, (2) fills the expandable body with **dressing advice derived from the forecast** (temp / rain-probability / wind / UV — there is no standalone free clothing API, so `dress()` rule-maps Open-Meteo metrics to Hebrew advice) plus 🌅/🌇 sun times, and (3) hides the static notes (`data-wx-static`). **Fallback is the whole point:** on offline, 7s timeout, or a date beyond Open-Meteo's ~16-day window (no `daily` in response), nothing changes and the static Hebrew notes remain. Far-future days show static now and flip to live as the trip nears — expected, not a bug.
